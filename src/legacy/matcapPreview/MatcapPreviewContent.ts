...
import type MatcapEditorWorld from '@/legacy/matcapPreview/MatcapPreviewWorld';
...
class MatcapPreviewContent {
	...
	public addObject(object: Object3D) {
		...
		object.traverse((child) => {
			if (child instanceof Mesh) {
				...
				if (originMaterial.color) child.material.color2 = originMaterial.color;
	...
	private onSnapshotsReady = (snapshots: {
		...
		this.matcapLoader.load(snapshots.matcap, (texture: Texture) => {
			...
			this._meshes.forEach((mesh: Mesh) => {
				(mesh.material as MeshMatcapORMMaterial).color.setScalar(
					this._store.power
				);
	...
	private onObjectPowerUpdate = (): void => {
		if (this._selectedMesh) {
			(this._selectedMesh.material as MeshMatcapORMMaterial).color.setScalar(
				this._store.power
			);
		} else {
			this._meshes.forEach((mesh: Mesh) => {
	...
	private onObjectRoughnessUpdate = (): void => {
		if (this._selectedMesh) {
			(this._selectedMesh.material as MeshMatcapORMMaterial).roughness =
				this._store.roughness;
		} else {
			this._meshes.forEach((mesh: Mesh) => {
				(mesh.material as MeshMatcapORMMaterial).roughness =
					this._store.roughness;
			});
		}
	};
	...
	private onObjectMetalnessUpdate = (): void => {
		if (this._selectedMesh) {
			(this._selectedMesh.material as MeshMatcapORMMaterial).metalness =
				this._store.metalness;
		} else {
			this._meshes.forEach((mesh: Mesh) => {
				(mesh.material as MeshMatcapORMMaterial).metalness =
					this._store.metalness;
			});
		}
	};
